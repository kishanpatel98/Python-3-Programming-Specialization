...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        return str(input("Guess a letter, phrase, or type 'exit' or 'pass': "))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        guessOptions  = self.getPossibleLetters(guessed)

        if len(guessOptions) == 0:
            return 'pass'
        else:
            guessValue = self.smartCoinFlip()

            if guessValue == True:
                updatedOptionsLen = len(self.SORTED_FREQUENCIES) - 1

                while(0 <= updatedOptionsLen <= len(self.SORTED_FREQUENCIES) - 1):
                    if self.SORTED_FREQUENCIES[updatedOptionsLen] in guessOptions:
                        return self.SORTED_FREQUENCIES[updatedOptionsLen]
                    else:
                        updatedOptionsLen -= 1
                        continue
            else:
                return random.choice(guessOptions)
