...
passed = [score[0] for score in students if score[1] >= 70]

#Write code using zip and filter so that these lists (l1 and l2) are combined into one big list and assigned to the variable opposites if they are both longer than 3 characters each.
l1 = ['left', 'up', 'front']
l2 = ['right', 'down', 'back']

opposites = filter(lambda string: len(string[0]) > 3 and len(string[1]) > 3, zip(l1, l2))
...
endangered =  [animal for animal, quantity in pop_info if quantity < 2500]
print (endangered)
