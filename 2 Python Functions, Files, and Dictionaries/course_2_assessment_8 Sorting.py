...
top_three = sorted(medals.keys(), key = lambda count: medals.get(count), reverse = True)[:3]
...
most_needed = sorted(groceries.keys(), key = lambda count: groceries.get(count), reverse = True)
# most_needed = sorted(groceries.keys(), key = lambda count: groceries.get(count))[::-1]
...
lambda_sort = sorted(ex_lst, key = lambda ex_lst: ex_lst[1])
